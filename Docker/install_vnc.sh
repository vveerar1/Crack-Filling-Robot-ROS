...
session.styleFile: ~/.fluxbox/styles/Arc-Dark
...
[begin] (  Application Menu  )
    [submenu] (ROS 2 Tools)
        [exec] (ROS 2 Core) {tilix -t "ROS 2 Topics" -e bash -lc "source /opt/ros/humble/install/setup.bash && source /opt/ros/humble/install/setup.bash && ros2 daemon start; read -p 'Press Enter to close...'"}
        [exec] (Launch RViz2) {bash -c "source /opt/ros/humble/install/setup.bash && source /opt/ros/humble/install/setup.bash && rviz2"}
        [exec] (List ROS 2 Nodes) {tilix -t "ROS 2 Topics" -e bash -lc "source /opt/ros/humble/install/setup.bash && source /opt/ros/humble/install/setup.bash && ros2 node list; read -p 'Press Enter to close...'"}
        [exec] (List ROS 2 Topics) {tilix -t "ROS 2 Topics" -e bash -lc "source /opt/ros/humble/install/setup.bash && source /opt/ros/humble/install/setup.bash && ros2 topic list; read -p 'Press Enter to close...'"}
        [exec] (List ROS 2 RQt Graph) {bash -c "source /opt/ros/humble/install/setup.bash && source /opt/ros/humble/install/setup.bash && rqt_graph"}
    [end]
...
#!/bin/bash
RESOLUTION=\${1:-\${VNC_RESOLUTION:-1280x800}}
DPI=\${2:-\${VNC_DPI:-96}}
